...
{		
	...
	{			
		...
		/*if ( comm->msqid == -1 )
		{
		      if ( (comm->msqid = msgget(key, (IPC_CREAT | 0666))) < 0 )
		      {
			      if( alarmFlag )
			      {
				      alarm(0);
	      ...
	      //Es el server, inicializo message queue
	      if ( (comm->msqid = msgget(key, (IPC_CREAT | 0666))) < 0) 
	      {
		    free(comm);
		    fatal("Error server message queue");
	      }
...
liberar_conexion( commT comm )
{
	...
	if( comm->msqid != -1 )
	{
		msgctl(comm->msqid, IPC_RMID, (struct msqid_ds *)NULL);
		comm->msqid = -1;
	}
...
{	 
	...
	sbuf.mtype = 1;
	memcpy(sbuf.mtext, datos, dim);

	if( ( msgsnd(comm->msqid, &sbuf, dim, IPC_NOWAIT) < 0 ) )
	{	
		liberar_conexion( comm );
		return -1;
	}
...
recibir_datos( commT comm, void *datos, int lim)
{
	...
	if( (n = msgrcv(comm->msqid, &rbuf, lim, 1, 0)) <= 0 )
	{	
		liberar_conexion( comm );
		return -1;
	}
